...
const MY_INFO = {
    ...
    handleSlider: function(){
        ...
        function handleDragSlide(){
            _this.slideBar.onmousedown = 
            _this.slideBar.ontouchstart = function(e){
                clearInterval(setAutoNext);

                const xDragLocation = e.x || Math.floor(e.touches[0].clientX)
                const slideBarWidth = this.offsetWidth
                let deviant = 0

                this.onmousemove = 
                this.ontouchmove = function(e){
                    const currentXDragLocation = e.x || Math.floor(e.touches[0].clientX)
                    deviant = currentXDragLocation - xDragLocation
                    
                    this.style =  `transform: translateX( calc(-${indexSlide*100}% + ${deviant}px) )`
                }
                this.onmouseup = 
                this.onmouseleave =
